...
# at top of utils/tr_va_te_split.py (or wherever balance_train_only lives)
try:
    from imblearn.over_sampling import ADASYN, SMOTE
    from imblearn.combine import SMOTEENN
    from imblearn.under_sampling import EditedNearestNeighbours  # ← missing import
except Exception:
    ADASYN = SMOTE = SMOTEENN = EditedNearestNeighbours = None
...
# assuming these may be conditionally imported earlier in your module:
# from imblearn.over_sampling import ADASYN, SMOTE
# from imblearn.combine import SMOTEENN
...
def balance_train_only(
    ...
    **kwargs,
    ...
    Parameters (in addition to common kwargs like sampling_strategy):
      method="adasyn":
        - n_neighbors (int, default 5)

      method="smote":
        - k_neighbors or n_neighbors (int, default 5)  # either name accepted

      method="smoteenn":
        - smote_k_neighbors or k_neighbors or n_neighbors (int, default 5)
        - enn_n_neighbors (int, default 3)
        (any other kwargs are ignored for SMOTE/ENN unless you pass them to the
         specific inner objects below—customize here if needed)
    """
    ...
    if method == "adasyn":
        # ADASYN expects n_neighbors
        n = kwargs.pop("n_neighbors", 5)
        sampler = ADASYN(random_state=random_state, n_neighbors=n, **kwargs)

    elif method == "smote":
        # SMOTE expects k_neighbors (older docs sometimes say n_neighbors)
        k = kwargs.pop("k_neighbors", kwargs.pop("n_neighbors", 5))
        sampler = SMOTE(random_state=random_state, k_neighbors=k, **kwargs)

    elif method == "smoteenn":
        # Route neighbor args to inner samplers (SMOTE + ENN)
        smote_k = kwargs.pop("smote_k_neighbors",
                    kwargs.pop("k_neighbors",
                    kwargs.pop("n_neighbors", 5)))
        enn_n   = kwargs.pop("enn_n_neighbors", 3)

        # Pull out a few commonly-used kwargs that should go to SMOTE, if present
        smote_kwargs = {}
        for key in ("sampling_strategy", "random_state"):
            if key in kwargs:
                smote_kwargs[key] = kwargs.pop(key)

        # Build inner samplers
        smote = SMOTE(random_state=random_state, k_neighbors=smote_k, **smote_kwargs)
        enn   = EditedNearestNeighbours(n_neighbors=enn_n)

        # Remaining kwargs (if any) do not apply directly to SMOTEENN in most versions
        sampler = SMOTEENN(random_state=random_state, smote=smote, enn=enn)
    ...
    # Ensure DataFrame/Series with column/index names preserved when possible
...
def scale_train_val_test(
    ...
    *,
    scale_binary: bool = False  # set True if you really want to scale 0/1 columns
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, StandardScaler]:
    """
    Fit scaler on balanced-train numeric features (optionally excluding binary dummies),
    transform train/val/test, and return DataFrames with original column order.
    """
    if scaler is None:
    ...
    # 2) Optionally exclude binary (0/1 only) columns from scaling
    if not scale_binary:
        def is_binary(s: pd.Series) -> bool:
            vals = pd.unique(s.dropna())
            # treat as binary if subset of {0,1} (allow ints/floats that are 0.0/1.0)
            return len(vals) <= 2 and set(map(float, vals)).issubset({0.0, 1.0})
        bin_cols = [c for c in num_cols if is_binary(X_train_res[c])]
        num_cols = [c for c in num_cols if c not in bin_cols]

    # 3) Fit on train numeric cols only
    if num_cols:
        scaler.fit(X_train_res[num_cols])
        tr_scaled = pd.DataFrame(
            scaler.transform(X_train_res[num_cols]),
            columns=num_cols, index=X_train_res.index
        )
        va_scaled = pd.DataFrame(
            scaler.transform(X_val[num_cols]),
            columns=num_cols, index=X_val.index
        )
        te_scaled = pd.DataFrame(
            scaler.transform(X_test[num_cols]),
            columns=num_cols, index=X_test.index
        )
    else:
        # no numeric columns to scale
        tr_scaled = X_train_res[num_cols].copy()
        va_scaled = X_val[num_cols].copy()
        te_scaled = X_test[num_cols].copy()

    # 4) Reassemble full DataFrames in original column order
    def replace_cols(X_orig: pd.DataFrame, X_scaled_subset: pd.DataFrame) -> pd.DataFrame:
        X_new = X_orig.copy()
        for c in X_scaled_subset.columns:
            X_new[c] = X_scaled_subset[c]
        return X_new

    X_train_out = replace_cols(X_train_res, tr_scaled)
    X_val_out   = replace_cols(X_val,       va_scaled)
    X_test_out  = replace_cols(X_test,      te_scaled)
...
def data_preprocessing(
    ...
    *,
    ...
    """
    balance_kwargs = (balance_kwargs or {}).copy()  # don't mutate caller's dict
    ...
    # (neighbors routing handled inside balance_train_only)
