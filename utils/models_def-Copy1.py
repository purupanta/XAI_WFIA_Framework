...
except Exception:  # if xgboost optional
    XGBClassifier = None
...
def fit_xgb_safe(
    ...
    # Fallback to your pure CPU trainer (no early stopping)
...
def fit_tabnet_safe(
    ...
    def _fit_classifier(pretrain_model=None, bs=None, vbs=None, w=None):
        ...
        try:
            hist = tn.history
            best_epoch = int(np.argmax(hist.get('val_0_auc', [0])))
            best_auc = float(np.max(hist.get('val_0_auc', [0.0])))
            stamp(f"[TabNet] best_epoch={best_epoch}, best_val_auc={best_auc:.5f}, "
                  f"epochs_ran={len(hist.get('val_0_auc', []))}")
        except Exception:
            pass
